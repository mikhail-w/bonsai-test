...
const Map = () => {
  ...
  return (
    <Box>
      <HStack justifyContent="space-between" mb={4} boxShadow={'outline'}>
        <IconButton
          icon={<FaBars />}
          onClick={onOpen}
...
const libraries = ['places'];
const mapContainerStyle = {
  height: '80vh', // Increased the height to cover more screen space
  width: '100%',
};
...
const Map = () => {
  ...
  return (
    <Box display={{ md: 'flex' }} height="100vh">
      ...
      >
        <Box>
          ...
          <Heading mt={5} size="md" mb={4} fontFamily="rale">
            Nearby Bonsai Locations:
          </Heading>
        </Box>
        <VStack spacing={4} align="stretch">
    ...
    <Box display={{ md: 'flex' }} height="100vh">
      ...
      >
        ...
        <Box mt={4} boxShadow="outline">
          <List spacing={0}>
            {' '}
            {/* Removed spacing between list items */}
            {locationList.map(location => (
              <ListItem
                key={location.place_id}
...
export default Map;
**********************************extra space below map in mobile mode***************
import React, { useState, useEffect, useRef } from 'react';
import {
  GoogleMap,
  useLoadScript,
